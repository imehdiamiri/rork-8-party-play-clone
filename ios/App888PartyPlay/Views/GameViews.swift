...
struct GameSessionView: View {
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        placeholderBody(session: session)
                    }
                    .padding(.horizontal, 16)
                    .padding(.top, 12)
                    .padding(.bottom, 24)
    ...
    private func sessionHeader(session: GameSession) -> some View {
        SurfaceCard {
            VStack(alignment: .leading, spacing: 10) {
                HStack(alignment: .top) {
                    VStack(alignment: .leading, spacing: 4) {
                    ...
                    Spacer()
                    StatusPillView(title: session.mode.title, systemImage: session.mode.icon, tint: session.mode.accentColor)
                }
                HStack(spacing: 6) {
                    MetricChipView(title: "Round \(min(session.currentRoundIndex + 1, session.rounds.count))/\(session.rounds.count)", systemImage: "flag.fill")
                    MetricChipView(title: "Timer \(session.secondsRemaining)s", systemImage: "timer")
                }
            }
        }
    }

    private func placeholderBody(session: GameSession) -> some View {
        SurfaceCard {
            VStack(alignment: .leading, spacing: 12) {
                SectionHeaderView(title: "Game Module Placeholder", subtitle: "Session infrastructure remains, game logic plugs in here.")
                Text(session.rounds[safe: session.currentRoundIndex]?.prompt ?? "No prompt yet.")
                    .font(.title3.weight(.semibold))
                Text(session.latestFeedback.isEmpty ? "This area is ready for the next game module." : session.latestFeedback)
                    .font(.footnote)
                    .foregroundStyle(.secondary)
                ...
                VStack(spacing: 8) {
                    Button("Continue") {
                        if session.phase == .finished {
                            ...
                            appModel.completeRound(success: true)
                        }
                    }
                    .buttonStyle(PrimaryActionButtonStyle())
