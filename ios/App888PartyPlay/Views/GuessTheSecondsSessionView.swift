...
struct GuessTheSecondsSessionView: View {
    ...
    @State private var multiIsRunning: Bool = false
    @State private var multiStartedAt: Date?
    ...
    private var multiDeviceBody: some View {
        if let gts = appModel.activeSession?.guessTheSecondsState {
            ...
            let playerCount = currentSession.players.count
            let roundNumber = gts.currentRoundNumber(playerCount: playerCount)
            let targetLocked = gts.roundTargets[roundNumber] != nil
            let displayTarget = gts.roundTargets[roundNumber] ?? multiSelectedTime
            let canEditTarget = !multiIsRunning && !gts.isFinished && gts.isFirstPlayerOfRound(playerCount: playerCount) && !targetLocked && isMyTurn

            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    SurfaceCard {
                        HStack(spacing: 10) {
                            if let name = turnPlayerName, !gts.isFinished {
                                HStack(spacing: 7) {
                                    Text("Round \(roundNumber) / \(gts.roundsPerPlayer)")
                                        .font(.caption.weight(.semibold))
                                        .foregroundStyle(.secondary)
                                    CurrentTurnPill(playerName: name, prefix: "Now", accent: .green)
                                }
                            } else {
                                Text(gts.isFinished ? "All rounds complete" : "Waiting...")
                                    .font(.caption.weight(.semibold))
                                    .foregroundStyle(.secondary)
                            }
                            Spacer(minLength: 0)
                            StatusPillView(
                                title: gts.isFinished ? "Finished" : (isMyTurn && multiIsRunning ? "Running" : (isMyTurn ? "Your Turn" : "Waiting")),
                                systemImage: gts.isFinished ? "checkmark.seal.fill" : (multiIsRunning ? "timer" : "figure.mind.and.body"),
                                tint: gts.isFinished ? .green : (isMyTurn ? .blue : .secondary)
                            )
                        }
                    }
                    ...
                    if !gts.isFinished {
                        ...
                        SurfaceCard {
                            VStack(alignment: .leading, spacing: 18) {
                                if isMyTurn {
                                    SectionHeaderView(title: "Target Time", subtitle: targetLocked ? "Target locked for this round." : "Set the target for this round.")
                                    multiSelectorArea(displayTarget: displayTarget, canEdit: canEditTarget)
                                    VStack(spacing: 14) {
                                        if !multiIsRunning {
                                            Button {
                                                multiStartTurn(gts: gts, playerCount: playerCount)
                                            } label: {
                                        ...
                                        .disabled(!multiIsRunning)
                                    }
                                    .sensoryFeedback(.success, trigger: multiCompletedCount)
                                    .sensoryFeedback(.selection, trigger: multiIsRunning)
                                } else {
                                    VStack(spacing: 16) {
                        ...
                        .animation(reduceMotion ? .linear(duration: 0.01) : .snappy, value: multiIsRunning)
                    }

                    multiScoreTableCard(gts: gts, players: currentSession.players)

                    if gts.isFinished {
                        multiResultsCard(gts: gts, players: currentSession.players)
                    }
                }
                .padding(.horizontal, 16)
                .padding(.top, 8)
    ...
    private func multiSelectorArea(displayTarget: Double, canEdit: Bool) -> some View {
        HStack(spacing: 12) {
            GuessStepperButton(systemImage: "minus", isEnabled: canEdit) {
                multiSelectedTime = max(multiSelectedTime - 1, 1)
            } onRepeat: { _ in
                multiSelectedTime = max(multiSelectedTime - 1, 1)
            }
            VStack(spacing: 8) {
                Text(multiIsRunning ? "•••••" : formatSec(displayTarget))
                    .font(.system(size: 52, weight: .heavy, design: .default).width(.compressed))
                    .monospacedDigit()
                    .foregroundStyle(multiIsRunning ? .secondary : .primary)
                    .frame(maxWidth: .infinity)
            }
            .frame(maxWidth: .infinity)
            .padding(.vertical, 28)
            ...
            .overlay { RoundedRectangle(cornerRadius: 24).strokeBorder(.white.opacity(0.06)) }
            GuessStepperButton(systemImage: "plus", isEnabled: canEdit) {
                multiSelectedTime = min(multiSelectedTime + 1, 60)
            } onRepeat: { _ in
                multiSelectedTime = min(multiSelectedTime + 1, 60)
            }
        }
    ...
    private func multiStartTurn(gts: GuessTheSecondsGameState, playerCount: Int) {
        let roundNumber = gts.currentRoundNumber(playerCount: playerCount)
        if gts.roundTargets[roundNumber] == nil {
            let locked = ((multiSelectedTime * 100).rounded()) / 100
            appModel.setGTSTargetTime(locked, forRound: roundNumber)
        }
        multiStartedAt = Date()
        multiElapsedTime = 0
        multiIsRunning = true
    ...
    private func multiScoreTableCard(gts: GuessTheSecondsGameState, players: [PlayerProfile]) -> some View {
        SurfaceCard {
            VStack(alignment: .leading, spacing: 14) {
                SectionHeaderView(title: "Progress", subtitle: "Each round uses one shared target.")
                HStack(alignment: .top, spacing: 8) {
                    Text("Time")
                        .font(.caption.weight(.semibold))
                        .foregroundStyle(.secondary)
                        .frame(width: 72, alignment: .leading)
                    ForEach(1...max(gts.roundsPerPlayer, 1), id: \.self) { round in
                        VStack(spacing: 4) {
                            Text("R\(round)")
                                .font(.caption2.weight(.semibold))
                                .foregroundStyle(.secondary)
                            Text(gts.roundTargets[round].map { formatSec($0) } ?? "—")
                ...
                VStack(spacing: 10) {
                    ForEach(players) { player in
                        let playerResults = gts.turnResults.filter { $0.playerID == player.id }
                        let isCurrent = appModel.currentTurnPlayerName(in: appModel.activeSession ?? session) == player.username && !gts.isFinished
                        let cells: [GuessTheSecondsSessionViewModel.ScoreCell] = (1...max(gts.roundsPerPlayer, 1)).map { round in
                            guard let result = playerResults.first(where: { $0.round == round }) else {
                                return GuessTheSecondsSessionViewModel.ScoreCell(id: "\(player.id)-\(round)", text: "—", tint: .secondary, isEmpty: true)
                            }
                            let band = GuessTheSecondsSessionViewModel.AccuracyBand(difference: result.difference)
                            return GuessTheSecondsSessionViewModel.ScoreCell(id: "\(player.id)-\(round)", text: formatSec(result.actualTime), tint: band.tint, isEmpty: false)
                        }
                        let total = playerResults.reduce(0.0) { $0 + $1.difference }
                        let row = GuessTheSecondsSessionViewModel.ScoreRow(id: player.id, playerName: player.username, roundCells: cells, totalText: formatDiff(total), isCurrent: isCurrent)
                        scoreCompactRow(row)
                    }
                }
            }
    ...
    private func multiResultsCard(gts: GuessTheSecondsGameState, players: [PlayerProfile]) -> some View {
        let ranking: [(id: UUID, name: String, total: Double, avg: Double)] = players.map { player in
            let pr = gts.turnResults.filter { $0.playerID == player.id }
            let total = pr.reduce(0.0) { $0 + $1.difference }
            let avg = pr.isEmpty ? 0 : total / Double(pr.count)
            return (id: player.id, name: player.username, total: total, avg: avg)
        ...
        return SurfaceCard {
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .top) {
                    ...
                    Spacer(minLength: 0)
                    if let winner = ranking.first {
                        StatusPillView(title: winner.name, systemImage: "crown.fill", tint: .yellow)
                    }
                }
                VStack(spacing: 10) {
                    ForEach(Array(ranking.enumerated()), id: \.element.id) { index, result in
                        HStack(spacing: 12) {
                            Text("#\(index + 1)")
                            ...
                            VStack(alignment: .leading, spacing: 3) {
                                Text(result.name)
                                    .font(.headline.weight(.semibold))
                                Text("Avg \(formatDiff(result.avg))")
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
                            }
                            Spacer(minLength: 0)
                            Text(formatDiff(result.total))
                                .font(.title3.weight(.bold))
                                .monospacedDigit()
                        ...
                        .overlay { RoundedRectangle(cornerRadius: 16).strokeBorder(index == 0 ? .green.opacity(0.2) : .white.opacity(0.05)) }
                    }
                }
            }
        }
