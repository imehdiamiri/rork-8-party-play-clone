-- ============================================================
-- PartyGames — Complete Production Supabase Schema
-- Merged: base schema + casual guest room system
-- Safe to run in Supabase SQL Editor (idempotent, no dollar-quote issues)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.star_transactions (
    ...
    CONSTRAINT star_transactions_type_check CHECK (
        transaction_type IN (
            'reward', 'purchase', 'unlockPurchase',
            'gameReward', 'subscriptionGrant', 'refund'
        )
    )
);
...
-- Safe constraint adds (drop first if exists, then recreate)
ALTER TABLE public.casual_rooms DROP CONSTRAINT IF EXISTS uq_casual_rooms_room_code;
ALTER TABLE public.casual_rooms ADD CONSTRAINT uq_casual_rooms_room_code UNIQUE (room_code);

ALTER TABLE public.casual_room_players DROP CONSTRAINT IF EXISTS uq_casual_player_name_per_room;
ALTER TABLE public.casual_room_players ADD CONSTRAINT uq_casual_player_name_per_room UNIQUE (room_id, normalized_display_name);

ALTER TABLE public.casual_room_players DROP CONSTRAINT IF EXISTS uq_casual_player_session_per_room;
ALTER TABLE public.casual_room_players ADD CONSTRAINT uq_casual_player_session_per_room UNIQUE (room_id, session_token);
...
CREATE INDEX IF NOT EXISTS idx_casual_room_players_last_seen ON public.casual_room_players (last_seen_at);

-- ============================================================
-- 3. TRIGGERS — updated_at auto-set
-- ============================================================
        ...
        WHERE r.id = p_room_id
          AND (
              ...
              OR EXISTS (
                  ...
                  WHERE ri.room_id = r.id AND ri.invited_user_id = p_user_id
...
BEGIN
    ...
    RETURN QUERY
    SELECT
        p.id, p.username, p.email, p.public_id, p.avatar_url,
        CASE
            WHEN EXISTS (
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1 FROM public.friendships f
        WHERE f.user_id = least(v_uid, p_receiver_id) AND f.friend_id = greatest(v_uid, p_receiver_id)
    ) THEN RETURN; END IF;
...
BEGIN
    ...
    ) THEN RAISE EXCEPTION 'Only session owner can finalize'; END IF;
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM public.game_sessions gs
        WHERE gs.id = p_session_id AND gs.created_by = v_uid
    ) THEN RAISE EXCEPTION 'Only session owner can distribute rewards'; END IF;
...
-- ============================================================
-- 9. RPC FUNCTION — Account Deletion (App Store requirement)
-- ============================================================
...
CREATE POLICY wallets_insert_own ON public.wallets FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id AND stars_balance = 100);

-- Star Transactions
DROP POLICY IF EXISTS star_transactions_select_own ON public.star_transactions;
CREATE POLICY star_transactions_select_own ON public.star_transactions FOR SELECT TO authenticated USING (auth.uid() = user_id);

-- XP Progress
DROP POLICY IF EXISTS xp_progress_select_own ON public.xp_progress;
CREATE POLICY xp_progress_select_own ON public.xp_progress FOR SELECT TO authenticated USING (auth.uid() = user_id);

-- Game Trials
DROP POLICY IF EXISTS game_trials_select_own ON public.game_trials;
CREATE POLICY game_trials_select_own ON public.game_trials FOR SELECT TO authenticated USING (auth.uid() = user_id);

-- Game Unlocks
DROP POLICY IF EXISTS game_unlocks_select_own ON public.game_unlocks;
CREATE POLICY game_unlocks_select_own ON public.game_unlocks FOR SELECT TO authenticated USING (auth.uid() = user_id);
...
CREATE POLICY friendships_select_participant ON public.friendships FOR SELECT TO authenticated USING (auth.uid() = user_id OR auth.uid() = friend_id);
CREATE POLICY friendships_delete_participant ON public.friendships FOR DELETE TO authenticated USING (auth.uid() = user_id OR auth.uid() = friend_id);

-- Friend Requests
DROP POLICY IF EXISTS friend_requests_select_participant ON public.friend_requests;
CREATE POLICY friend_requests_select_participant ON public.friend_requests FOR SELECT TO authenticated USING (auth.uid() = sender_id OR auth.uid() = receiver_id);
...
CREATE POLICY rooms_select_visible ON public.rooms FOR SELECT TO authenticated
    USING (access = 'public' OR host_user_id = auth.uid() OR public.room_is_visible_to_user(id, auth.uid()));
CREATE POLICY rooms_insert_host ON public.rooms FOR INSERT TO authenticated WITH CHECK (auth.uid() = host_user_id);
...
CREATE POLICY room_members_select_visible ON public.room_members FOR SELECT TO authenticated
...
CREATE POLICY room_members_insert_self ON public.room_members FOR INSERT TO authenticated
    WITH CHECK (auth.uid() = user_id AND public.room_is_visible_to_user(room_id, auth.uid()));
CREATE POLICY room_members_update_self_or_host ON public.room_members FOR UPDATE TO authenticated
...
CREATE POLICY room_members_delete_self_or_host ON public.room_members FOR DELETE TO authenticated
    USING (auth.uid() = user_id OR public.is_room_host(room_id, auth.uid()));
...
CREATE POLICY room_invites_select_participant ON public.room_invites FOR SELECT TO authenticated
    USING (auth.uid() = inviter_user_id OR auth.uid() = invited_user_id);
CREATE POLICY room_invites_insert_inviter ON public.room_invites FOR INSERT TO authenticated
    WITH CHECK (auth.uid() = inviter_user_id AND public.is_room_host(room_id, auth.uid()));
CREATE POLICY room_invites_update_participant ON public.room_invites FOR UPDATE TO authenticated
    USING (auth.uid() = inviter_user_id OR auth.uid() = invited_user_id)
...
CREATE POLICY game_sessions_select_visible ON public.game_sessions FOR SELECT TO authenticated
    USING (room_id IS NOT NULL AND public.room_is_visible_to_user(room_id, auth.uid()));
CREATE POLICY game_sessions_insert_room_member ON public.game_sessions FOR INSERT TO authenticated
    WITH CHECK (auth.uid() = created_by AND room_id IS NOT NULL AND public.room_is_visible_to_user(room_id, auth.uid()));
CREATE POLICY game_sessions_update_creator ON public.game_sessions FOR UPDATE TO authenticated
...
CREATE POLICY game_results_select_room_member ON public.game_results FOR SELECT TO authenticated
    USING (EXISTS (SELECT 1 FROM public.game_sessions gs WHERE gs.id = game_results.session_id AND gs.room_id IS NOT NULL AND public.room_is_visible_to_user(gs.room_id, auth.uid())));
CREATE POLICY game_results_insert_creator ON public.game_results FOR INSERT TO authenticated
    WITH CHECK (EXISTS (SELECT 1 FROM public.game_sessions gs WHERE gs.id = game_results.session_id AND gs.created_by = auth.uid()));
CREATE POLICY game_results_update_creator ON public.game_results FOR UPDATE TO authenticated
    USING (EXISTS (SELECT 1 FROM public.game_sessions gs WHERE gs.id = game_results.session_id AND gs.created_by = auth.uid()))
    WITH CHECK (EXISTS (SELECT 1 FROM public.game_sessions gs WHERE gs.id = game_results.session_id AND gs.created_by = auth.uid()));

-- Unlock Items
DROP POLICY IF EXISTS unlock_items_select_all ON public.unlock_items;
CREATE POLICY unlock_items_select_all ON public.unlock_items FOR SELECT TO authenticated USING (true);

-- User Unlocks
DROP POLICY IF EXISTS user_unlocks_select_own ON public.user_unlocks;
CREATE POLICY user_unlocks_select_own ON public.user_unlocks FOR SELECT TO authenticated USING (auth.uid() = user_id);

-- Reward Idempotency
DROP POLICY IF EXISTS reward_idempotency_owner_only ON public.reward_idempotency;
CREATE POLICY reward_idempotency_owner_only ON public.reward_idempotency FOR SELECT TO authenticated USING (auth.uid() = owner_user_id);

-- Casual Rooms (guest system, anon-readable, mutations via RPCs only)
...
DO $rt_rooms$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.rooms; EXCEPTION WHEN duplicate_object THEN NULL; END $rt_rooms$;
DO $rt_members$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.room_members; EXCEPTION WHEN duplicate_object THEN NULL; END $rt_members$;
DO $rt_invites$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.room_invites; EXCEPTION WHEN duplicate_object THEN NULL; END $rt_invites$;
DO $rt_friends$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.friendships; EXCEPTION WHEN duplicate_object THEN NULL; END $rt_friends$;
DO $rt_freq$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.friend_requests; EXCEPTION WHEN duplicate_object THEN NULL; END $rt_freq$;
DO $rt_gsess$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.game_sessions; EXCEPTION WHEN duplicate_object THEN NULL; END $rt_gsess$;
DO $rt_cas_rooms$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.casual_rooms; EXCEPTION WHEN duplicate_object THEN NULL; END $rt_cas_rooms$;
DO $rt_cas_players$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.casual_room_players; EXCEPTION WHEN duplicate_object THEN NULL; END $rt_cas_players$;
...
GRANT EXECUTE ON FUNCTION public.casual_create_room(uuid, text, text, text, uuid, text, text, integer, integer, integer, integer, integer, text) TO anon;
GRANT EXECUTE ON FUNCTION public.casual_create_room(uuid, text, text, text, uuid, text, text, integer, integer, integer, integer, integer, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.casual_join_room(text, uuid, text, text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.casual_join_room(text, uuid, text, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.casual_leave_room(uuid, uuid, text) TO anon;
GRANT EXECUTE ON FUNCTION public.casual_leave_room(uuid, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.casual_kick_player(uuid, uuid, text) TO anon;
GRANT EXECUTE ON FUNCTION public.casual_kick_player(uuid, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.casual_update_room_status(uuid, text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.casual_update_room_status(uuid, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.casual_update_room_settings(uuid, text, integer, integer, integer, text) TO anon;
GRANT EXECUTE ON FUNCTION public.casual_update_room_settings(uuid, text, integer, integer, integer, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.casual_heartbeat(uuid, text) TO anon;
GRANT EXECUTE ON FUNCTION public.casual_heartbeat(uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.casual_mark_disconnected(uuid, uuid, text) TO anon;
GRANT EXECUTE ON FUNCTION public.casual_mark_disconnected(uuid, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.casual_cleanup_stale_players(uuid, integer) TO anon;
GRANT EXECUTE ON FUNCTION public.casual_cleanup_stale_players(uuid, integer) TO authenticated;
...
-- ============================================================
-- DONE. All tables, functions, RLS, realtime, and grants are set.
-- ============================================================
